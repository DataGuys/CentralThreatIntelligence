...
echo -e "\n======================================================="; echo "     CTI Application Registration Setup Script"; echo "=======================================================\n"
echo -e "${BLUE}Checking prerequisites...${NC}"
if ! az account show &> /dev/null; then echo -e "${YELLOW}Not logged in to Azure.${NC}" && az login; fi
SUB_NAME=$(az account show --query name -o tsv); SUB_ID=$(az account show --query id -o tsv); TENANT_ID=$(az account show --query tenantId -o tsv)
echo -e "${GREEN}Using subscription: ${SUB_NAME} (${SUB_ID})${NC}"; echo -e "${GREEN}Tenant ID: ${TENANT_ID}${NC}"
echo -e "${BLUE}Creating Microsoft Entra ID application registration...${NC}"
read -p "Enter a name for the application [CTI-Solution]: " APP_NAME; APP_NAME=${APP_NAME:-"CTI-Solution"}
...
echo -e "${GREEN}Application successfully created.${NC}"; echo -e "${GREEN}Application (Client) ID: ${APP_ID}${NC}"
echo "Creating service principal for the application..."
az ad sp create --id $APP_ID > /dev/null
echo -e "${GREEN}Service principal created successfully.${NC}"
echo "CLIENT_ID=${APP_ID}" > cti-app-credentials.env; echo "APP_OBJECT_ID=${OBJECT_ID}" >> cti-app-credentials.env; echo "APP_NAME=${APP_NAME}" >> cti-app-credentials.env
...
read -p "Client secret duration in years [2]: " SECRET_YEARS; SECRET_YEARS=${SECRET_YEARS:-2}
...
echo -e "Keep this file secure as it contains your client secret!\n"
EOF
)"

...
RG='CTI-ResourceGroup'; LOC='eastus'; WS='CTI-Workspace'
ENABLE_SENTINEL=true ENABLE_MDTI=true ENABLE_COPILOT=false
...
# ---------- deploy -----------------------------------------------------------
az group create -n "$RG" -l "$LOC" --tags solution=CTI createdBy=deploy.sh --only-show-errors
curl -sSL "$RAW" -o main.bicep
az deployment group create -g "$RG" \
  --name "cti-$(date +%s)" \
  --template-file main.bicep \
  --parameters clientId="$CLIENT_ID" workspaceName="$WS" \
               enableSentinel=$ENABLE_SENTINEL enableMDTI=$ENABLE_MDTI \
